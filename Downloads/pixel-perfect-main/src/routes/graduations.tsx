...
const MILESTONES = [
  {
    ...
    body: "Every cap thrown in the air started here — with courage, a fresh notebook, and a blank page.",
    image: "/images/Graduation/IMG_3416.JPG",
  },
  {
    icon: Trophy,
    year: "The Grind",
    label: "Late Nights & Long Days",
    body: "The years between start and finish that no one photographs — but everyone feels.",
    image: "/images/Graduation/IMG_3419.jpg",
  },
  {
    icon: GraduationCap,
    year: "Final Year",
    label: "Almost There",
    body: "Pressure, pride, and anticipation building toward a single unforgettable morning.",
    image: "/images/Graduation/DJAY9788.jpg",
  },
  {
    icon: Camera,
    year: "Graduation Day",
    label: "You Made It.",
    body: "The handshake. The scroll. The photograph that hangs on walls for decades.",
    image: "/images/Graduation/DJAY9736.jpg",
  },
];
...
function GraduationsPage() {
  ...
  const stackItems = (cat.stackPhotos ?? cat.gallery.slice(0, 5)).map(
    (src, i) => ({ src, name: `Graduation ${i + 1}` })
  );
      ...
      <section ref={heroRef} className="relative h-screen min-h-[640px] overflow-hidden">
        ...
        >
          <motion.h1
            ...
            className="font-playfair text-[52px] leading-[0.9] text-white md:text-[84px] lg:text-[108px]"
          >
            Pride.
            <br />
            <span className="font-cormorant italic text-dj-warm">Achievement.</span>
            <br />
            Memory.
          </motion.h1>
          ...
          <motion.p
            ...
            className="mt-8 max-w-md font-cormorant text-[19px] leading-relaxed text-white/75"
          >
            {cat.description}
          </motion.p>
        </motion.div>
        ...
        <div className="mx-auto max-w-[1200px] px-6 lg:px-10">
          <div className="grid items-center gap-16 md:grid-cols-2">
            ...
            >
              ...
              <h2 className="font-playfair text-[40px] leading-[1.05] text-dj-ink md:text-[54px]">
                Moments that{" "}
                <span className="font-cormorant italic text-dj-warm">last forever.</span>
              </h2>
              <p className="mt-6 font-inter text-[14px] leading-[1.9] text-dj-mid">
                Every graduation session is as unique as the person in the frame. We capture the joy, the pride, the relief — and the quiet, in-between moments nobody else sees.
              </p>
              <p className="mt-4 font-inter text-[14px] leading-[1.9] text-dj-mid">
                Hover over the stack to explore a selection of our graduation work, then scroll down to see the full gallery.
              </p>
              <div className="mt-8 flex gap-8">
                {[
                  { n: "Full Gallery", label: "Below ↓" },
                  { n: "4 Weeks", label: "Delivery" },
                ].map((s) => (
                  <div key={s.label}>
      ...
      <section className="bg-dj-bg2 py-24 md:py-32 overflow-hidden">
        <div className="mx-auto max-w-[1300px] px-6 lg:px-10">
          <div className="mb-16 text-center">
            ...
            <h2 className="mt-4 font-playfair text-[36px] text-dj-ink md:text-[48px]">
              Every step{" "}
              <span className="font-cormorant italic text-dj-warm">matters.</span>
            </h2>
          </div>
            ...
            <div className="space-y-12">
              {MILESTONES.map((m, i) => {
                ...
                return (
                  <div
                    key={m.year}
                    className={`relative flex items-start gap-8 md:gap-0 ${isRight ? "md:flex-row" : "md:flex-row-reverse"}`}
                  >
                    {/* dot — plain div, outside animated card, never clipped */}
                    ...
                    {/* card — motion element with overflow-hidden */}
                    <motion.div
                      initial={{ opacity: 0, x: isRight ? -80 : 80 }}
                      whileInView={{ opacity: 1, x: 0 }}
                      viewport={{ once: true, amount: 0.2 }}
                      transition={{ duration: 0.8, delay: i * 0.08, ease: [0.22, 1, 0.36, 1] }}
                      className={`group relative ml-6 h-72 w-full max-w-sm overflow-hidden md:ml-0 md:h-80 md:w-[44%] ${
                    ...
                    >
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-6">
                        <p className="mb-1 font-inter text-[10px] uppercase text-white/60" style={{ letterSpacing: "0.2em" }}>
                          {m.year}
                        </p>
      ...
      <section className="bg-dj-bg2 py-24 text-center overflow-hidden">
        <div className="mx-auto max-w-[700px] px-6">
          ...
          <motion.h2
            ...
            className="font-playfair text-[32px] text-dj-ink md:text-[44px]"
          >
          ...
          <motion.p
            ...
            className="mx-auto mt-4 max-w-sm font-inter text-[14px] text-dj-mid"
          >
            Reach out early — graduation season fills up fast.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, x: 60 }}
            whileInView={{ opacity: 1, x: 0 }}
          ...
          >
            <Link
              to="/contact"
              className="inline-block border border-dj-ink bg-dj-ink px-12 py-4 font-inter text-[11px] uppercase text-dj-bg transition-all duration-300 hover:bg-transparent hover:text-dj-ink"
      ...
      <section className="bg-dj-bg py-20 overflow-hidden">
        <div className="mx-auto max-w-[1300px] px-6 lg:px-10">
          ...
          <div className="grid gap-1 md:grid-cols-3">
            {CATEGORIES.filter((c) => c.key !== "graduations")
              .slice(0, 3)
              .map((r, i) => {
...
export const Route = createFileRoute("/graduations")({
  head: () => ({
    meta: [
      { title: `Graduation Photography | Djay Photography` },
      { name: "description", content: cat.description },
      { property: "og:title", content: `Graduation Photography | Djay Photography` },
