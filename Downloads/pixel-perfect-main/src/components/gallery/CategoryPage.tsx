...
// ─── STEP IMAGES ARE COMPLETELY INDEPENDENT ───────────────────────────────────
// These are ONLY used in the "How It Works" process cards.
// They have nothing to do with gallery, stackPhotos, or hero images.
// Change these freely without affecting anything else on the page.
const STEPS = [
  {
    n: "01",
    title: "Consult",
    body: "We start with a conversation to understand your vision, venue, and the moments that matter most.",
    // ↓ Change these 3 images whenever you want — they only appear in this card
    images: [
      "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=400&q=80",
      "https://images.unsplash.com/photo-1554941426-88f2a00e8df1?w=400&q=80",
    ...
    <>
      ...
      <section className="relative flex h-screen min-h-[640px] items-center overflow-hidden">
        <div
          ...
          style={{ backgroundImage: `url(${category.hero})` }}
        />
      ...
      <section className="bg-dj-bg py-24 md:py-32">
        <div className="mx-auto max-w-[1500px] px-6 lg:px-10">
          ...
          <PhotoGallery
            photos={(category.stackPhotos ?? category.gallery.slice(0, 5)).map((src) => ({ src }))}
          />
      ...
      <section className="bg-dj-dark py-20 text-center">
        <h2 className="font-playfair text-[28px] text-dj-bg md:text-[36px]">
          Book Your {category.label} Session
        </h2>
        <Link
          to="/contact"
      ...
      <Lightbox
        ...
        onPrev={() => setOpen((i) => (i === null ? null : (i - 1 + category.gallery.length) % category.gallery.length))}
        onNext={() => setOpen((i) => (i === null ? null : (i + 1) % category.gallery.length))}
      />
    </>
  );
}